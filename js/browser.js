var UserAgents = (function(){
	...
	Detected.prototype = {
		...
		toString: function() {
			var prefix = this.camouflage ? '一个奇怪的浏览器, 看上去很像' : '';
			...
			if (this.device.identified)			
				...
				device += (typeof this.device.model != 'undefined' && this.device.model ? '未知设备 (' + this.device.model + ')' : '');
			...
			if (browser && os && device) {
				return prefix + browser + ', 运行在' + device + '上, 操作系统是' + os;
			}
		
			else if (browser && !os && device) {
				return prefix + browser + ', 运行在' + device + '上';
			}
		
			else if (browser && os && !device) {
				return prefix + browser + ', 操作系统是' + os;
			}
		
			else if (!browser && os && device) {
				return prefix + '运行在' + device + '上, 操作系统是' + os;
			}
				...
				return prefix + '运行在' + device;
			}
		
			else if (this.device.type == 'desktop' && os && engine != '' && !device) {
				return '一个奇怪的浏览器, 使用了' + engine + '引擎, 操作系统是' + os;
			}
